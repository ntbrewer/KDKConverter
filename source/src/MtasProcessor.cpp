...
//		    nrOfCentralPMTs++;
    ...
    unsigned long long int betaTimeHighRes=0;
    ...
    int    tTraceMax = -1.0;
    int    tRegion = -1.0;
    vector<int> trace; //Don't access this value, please.
    //int siSat = -1;
    //int siPileup = -1;
//     Matt** Loop over all APD. Sili means APD
	...
	{
	//siSat = (*siliListIt)->IsSaturated();
        //siPileup = (*siliListIt)->IsPileup();
        string subtype = (*siliListIt)->GetChanID().GetSubtype();
	trace = (*siliListIt)->GetTrace();
		if (mtasMap.count(subtype)>0)
	...
	if(siliMap.size()>0)
	{
            ...
            {
                isBetaSignal = true;
                betaTimeHighRes = siliEv->GetTime();
    ...
    unsigned long long int actualTimeHighRes = 0;
    unsigned long long int earliestRingTime[5];
	...
	if(mtasSummary->GetMult() > 0)//I have at leat one element in mtasList
	{
		...
		actualTimeHighRes = (*mtasListIt)->GetTime();
		if (firstTime == -1.) {
...
//spectras 
            ...
            mtasMapIt != mtasMap.end(); mtasMapIt++)
	{
		...
		double highResTime = (*mtasMapIt).second.time;
		...
		if((*mtasMapIt).first[0] == 'C')
		{
          ...
          if (earliestModuleTime[0] > time ) 
          {
              ...
              earliestRingTime[0]=highResTime;
          }
		...
		else if((*mtasMapIt).first[0] == 'I')
		{
	      ...
	      earliestRingTime[1]=highResTime;
          } 
		...
		else if((*mtasMapIt).first[0] == 'M')
		{
          ...
          {
              earliestModuleTime[moduleIndex-5]=time;
              earliestRingTime[2]=highResTime;
          } 
          if (earliestIMOTime > 0 && time < earliestIMOTime)
          {
            earliestIMOTime = time;
		...
		else if((*mtasMapIt).first[0] == 'O')
		{
          ...
          {
              earliestModuleTime[moduleIndex-5]=time;
              earliestRingTime[3]=highResTime;
          } 
          if (earliestIMOTime > 0 && time < earliestIMOTime)
          {
            earliestIMOTime = time;
	...
	for(unsigned int i=0; i<sumFrontBackEnergy.size(); i++)
	{
		...
		for(int i=0; i<6; i++)
		{
            ...
            /*### For KDK file output ###*/
	    //read in filenames from config/outputfile_names.txt
 	    //output to text "header" first time, then event_ID, actualTime, maxSiliconSignal, totalMtasEnergy.at(0) 
	    // output to event_ID, actualTime, traceLength, trace
	    static int event_ID=0;
            static char txtFileName[256]="";
            int eventType = (totalMtasEnergy.at(0) > 0 ) + 2*( maxSiliconSignal >0 );
	    std::ofstream outputFile;
	    std::stringstream ss; 
	    if (event_ID==0) 
	    {
            std::ifstream inputFile;
	    inputFile.open("source/config/outputfile_names.txt", std::ifstream::in);
            inputFile.getline(txtFileName,256);
            inputFile.close();
            //cout << "filename is: " << txtFileName << endl;
		ss << "Type\tId\tTf(s)\tEc\tTc\tEi\tTi\tEm\tTm\tEo\tTo\tEt\tTns\t";
                ss << "Eapd\tTapd\tLen\tTrace\t\n";
//                ss << "Eapd\tTapd\tPileup\tSaturation\tLen\tTrace\t\n";
	    /*write out */
	    outputFile.open(txtFileName,std::ofstream::out);
            outputFile << ss.rdbuf() << "\n";
            outputFile.close();
	 	...
	 	ss << actualTime << "\t";
 	     ...
 	     ss << totalMtasEnergy.at(1) << "\t";
	     ss << earliestRingTime[0] << "\t";
	     ss << totalMtasEnergy.at(2) << "\t";
	     ss << earliestRingTime[1] << "\t";
	     ss << totalMtasEnergy.at(3) << "\t";
	     ss << earliestRingTime[2] << "\t";
	     ss << totalMtasEnergy.at(4) << "\t";
	     ss << earliestRingTime[3] << "\t";
	     //ss << totalMtasEnergy.at(5) << "\t"; //TBD
	     ss << totalMtasEnergy.at(0) << "\t";
	     ss << actualTimeHighRes << "\t"; 
	     ss << maxSiliconSignal << "\t";
 	     ss << betaTimeHighRes << "\t"; //Be careful with this one
//	     ss << siPileup << "\t";
//	     ss << siSat << "\t";
...
//          const bool noCenter = totalMtasEnergy.at(1) < 1.0;
	...
	EndProcess(); // update the processing time
        outputFile.open(txtFileName,std::ofstream::out | std::ofstream::app);
        outputFile << ss.rdbuf() << "\n";
        outputFile.close();
